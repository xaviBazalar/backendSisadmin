...
const sendEmail=(email,nombre,usuario,urlApp)=>{
    var fec=new Date()
    var https = require('https');
    let hash_recovery=md5(fec.getTime()+"email")
        ...
        data:
          {
            ...
            html: `Haga click en el enlace para restablecer la constraseña.<a href='${urlApp}#/recoveryUsuario/${hash_recovery}'>Click Aqui</a>`,//"Su clave de acceso es: "+clave+" .Recuerde cambiar su clave para mayor seguridad.",
    ...
    const recoveryUsuario = new RecoveryUsuario({ usuario,hash_recovery });
    recoveryUsuario.save()
...
const loginGet = async(req = request, res = response) => {
    ...
    const { emailS="",url="" } = req.query;
    const query = { correo:emailS.toLowerCase()};

    const [ total, validation  ] = await Promise.all([
        Usuario.countDocuments(query),
        Usuario.find(query,{ "correo":1,"nombre":1,"password":1,"login":1}).
        populate( { path: "perfil",model:Perfil})
    ]);

    if(total===1){
        sendEmail(validation[0].correo,validation[0].nombre,validation[0].login,url)
    }
